...
class Grafico extends CI_Controller {
	...
	public function show()
	{
		try{
			...
			$ing = $this->db->query("SELECT SUM(mov_monto) FROM movimiento WHERE log_id = 3 AND mov_tipo_movimiento = 0 AND YEAR(mov_fecha) = YEAR(CURDATE()) AND MONTH(mov_fecha) = MONTH(CURDATE());")->row();
			$egr = $this->db->query("SELECT SUM(mov_monto) FROM movimiento WHERE log_id = 3 AND mov_tipo_movimiento = 1 AND YEAR(mov_fecha) = YEAR(CURDATE()) AND MONTH(mov_fecha) = MONTH(CURDATE());")->row();
